(function ($) {
    ...
    function MultipleSelect($el, options) {
        ...
        this.$el = $el.hide();
        this.$label = this.$el.closest("label") || this.$el.attr("id") && $(sprintf('label[for="%s"]', this.$el.attr("id").replace(/:/g, "\\:")));
    ...
    MultipleSelect.prototype = {
        constructor: MultipleSelect, init: function () {
            ...
            var that = this, $elm = $(elm), classes = $elm.attr("class") || "",
                title = sprintf('title="%s"', $elm.attr("title")), multiple = this.options.multiple ? "multiple" : "",
                disabled, type = this.options.single ? "radio" : "checkbox";
            if ($elm.is("option")) {
                var value = $elm.val(), text = that.options.textTemplate($elm), selected = $elm.prop("selected"),
                    style = sprintf('style="%s"', this.options.styler(value));
                disabled = groupDisabled || $elm.prop("disabled");
                return $([sprintf('<li class="%s %s" %s %s>', multiple, classes, title, style), sprintf('<label class="%s">', disabled ? "disabled" : ""), sprintf('<input type="%s" %s%s%s%s value="%s">', type, this.selectItemName, selected ? ' checked="checked"' : "", disabled ? ' disabled="disabled"' : "", sprintf(' data-group="%s"', group), value), text, "</label>", "</li>"].join(""))
            }
            if ($elm.is("optgroup")) {
                var group = "group_" + i, label = that.options.labelTemplate($elm), $group = $("<div/>");
                disabled = $elm.prop("disabled");
                $group.append(['<li class="group">', sprintf('<label class="optgroup %s" data-group="%s">', disabled ? "disabled" : "", group), this.options.hideOptgroupCheckboxes ? "" : sprintf('<input type="checkbox" %s %s>', this.selectGroupName, disabled ? 'disabled="disabled"' : ""), label, "</label>", "</li>"].join(""));
            ...
            var selects = this.options.displayValues ? this.getSelects() : this.getSelects("text"),
                $span = this.$choice.find(">span"), sl = selects.length;
            if (sl === 0) {
